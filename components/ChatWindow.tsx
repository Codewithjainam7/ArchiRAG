      ...
      <div ref={scrollRef} className="chat-scroll-area p-4 md:p-12 space-y-10 md:space-y-20 scroll-smooth">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col animate-message ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            <div className={`glass p-6 md:p-12 rounded-3xl md:rounded-[3rem] max-w-[98%] md:max-w-[90%] border-white/5 shadow-2xl transition-all duration-500 hover:border-blue-500/20 ${
              msg.role === 'user' ? 'bg-blue-600/5 border-blue-500/40 shadow-blue-500/5' : 'bg-[#0a0f18]/80'
            }`}>
              ...
              {msg.role === 'assistant' && (
                <div className="mt-10 pt-8 border-t border-white/5 flex flex-wrap gap-4">
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-3 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all px-6 py-3 rounded-2xl border ${
                      isGeneratingCards === msg.id 
        ...
        <div className="max-w-5xl mx-auto">
          <div className="glass rounded-[2.5rem] p-2.5 flex items-center space-x-3 border-white/10 shadow-2xl focus-within:border-blue-500/50 transition-all duration-700 hover:border-white/20">
            <textarea
              ...
              className="flex-1 bg-transparent border-none outline-none text-sm md:text-base px-6 py-5 resize-none h-18 md:h-20 text-slate-100 placeholder-slate-700 font-medium scrollbar-hide"
              disabled={isLoading || documentsCount === 0}
            />
            <button 
              onClick={() => handleSend()} 
              disabled={!input.trim() || isLoading || documentsCount === 0}
              className={`p-6 rounded-[1.8rem] transition-all duration-500 ${!input.trim() || isLoading ? 'text-slate-800 bg-white/5' : 'bg-blue-600 text-white active:scale-90 hover:bg-blue-500 shadow-2xl shadow-blue-500/40 hover:rotate-6'}`}
            >
